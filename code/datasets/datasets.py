...
class ShapeNetDataSet(data.Dataset):
    ...
    def __init__(self,with_gt = False,with_scans=False,**kwargs):
        ...
        #os.path.join(os.path.split(base_dir)[0], 'shapenetDataset')
        self.scans_files = self.get_instance_filenames('./demo/shapenet',kwargs['split'], '', kwargs['scans_file_type'] if 'scans_file_type' in kwargs else 'obj') if with_scans else None
        if (with_gt):
            # Used only for evaluation #/media/digiaires/8TBHDD1/ShapeNetCorev2 #os.path.join(os.path.split(base_dir)[0],'shapenetDataset'
            self.gt_files = self.get_instance_filenames('./demo/shapenet',kwargs['split'],'','obj')
    ...
    def get_instance_filenames(self,base_dir,split,ext='',format='npy'):
        ...
        for dataset in split:
            for class_name in split[dataset]:
                for model_name in split[dataset][class_name]:
                    for instance_name in split[dataset][class_name][model_name]:
                        j = 0
                        for shape in split[dataset][class_name][model_name][instance_name]:

                            instance_filename = os.path.join(base_dir, class_name,model_name,instance_name, shape + "{0}.{1}".format(ext,format))
