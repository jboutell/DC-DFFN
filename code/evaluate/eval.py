...
import logging
from datasets.datasets import ShapeNetDataSet
import torch
...
def evaluate_with_load(gpu, conf, exps_folder_name, override, timestamp, checkpoint ,parallel, resolution, recon_only=False, plot_cmpr=False,with_opt=False):
...
def evaluate(network,exps_folder_name, experiment_name, timestamp, epoch, resolution, conf, index, recon_only,lat_vecs,plot_cmpr=False,with_gt=False, with_opt=False, recon_chamfer=False):
    ...
    print('Recon_only:' + str(recon_only))
    if not recon_only:
